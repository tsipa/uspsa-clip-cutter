"""Fuzzy phrase matching over transcript segments.

Uses greedy longest-match-first: longer phrases claim word indices before
shorter ones, preventing e.g. "holster" from matching standalone when it's
already part of "hammer down and holster".
"""

from __future__ import annotations

import logging
from dataclasses import dataclass, field

from rapidfuzz import fuzz

from video_stage_cutter.transcribe import TranscriptSegment

log = logging.getLogger(__name__)

START_PHRASES = [
    "load and make ready",
    "are you ready",
    "stand by",
    "standby",
    "make ready",
]

END_PHRASES = [
    "if you are finished unload and show clear",
    "unload and show clear",
    "if clear hammer down and holster",
    "if clear hammer down",
    "hammer down and holster",
    "hammer down holster",
    "hammer down",
    "range is clear",
]

DEFAULT_THRESHOLD = 70
MAX_WORD_GAP = 2.0


@dataclass
class PhraseMatch:
    start: float
    end: float
    text: str
    score: float
    matched_phrase: str
    role: str  # "start" or "end"


@dataclass
class _RawMatch:
    """Internal: a match with word index range for greedy dedup."""
    word_start: int
    word_end: int  # exclusive
    match: PhraseMatch
    phrase_len: int = 0


def detect_phrases(
    segments: list[TranscriptSegment],
    threshold: float = DEFAULT_THRESHOLD,
) -> tuple[list[PhraseMatch], list[PhraseMatch]]:
    """Find start-phrase and end-phrase candidates in *segments*.

    Returns ``(start_matches, end_matches)`` sorted by timestamp.
    """
    all_words: list[str] = []
    all_starts: list[float] = []
    all_ends: list[float] = []

    for seg in segments:
        for w in seg.words:
            cleaned = w.word.strip().strip(".,!?;:'\"").lower()
            if cleaned:
                all_words.append(cleaned)
                all_starts.append(w.start)
                all_ends.append(w.end)

    if not all_words:
        segment_text = " ".join(s.text for s in segments).lower()
        return _fallback_segment_match(segments, segment_text, threshold)

    # collect all raw matches with word index ranges
    raw: list[_RawMatch] = []

    for phrase in START_PHRASES:
        raw.extend(_find_raw_matches(
            phrase, all_starts, all_ends, all_words, threshold, "start",
        ))
    for phrase in END_PHRASES:
        raw.extend(_find_raw_matches(
            phrase, all_starts, all_ends, all_words, threshold, "end",
        ))

    # greedy: longest phrase first, then highest score
    raw.sort(key=lambda r: (r.phrase_len, r.match.score), reverse=True)

    used_indices: set[int] = set()
    accepted: list[PhraseMatch] = []

    for r in raw:
        word_indices = set(range(r.word_start, r.word_end))
        if word_indices & used_indices:
            continue
        used_indices |= word_indices
        accepted.append(r.match)

    start_matches = [m for m in accepted if m.role == "start"]
    end_matches = [m for m in accepted if m.role == "end"]

    start_matches.sort(key=lambda m: m.start)
    end_matches.sort(key=lambda m: m.start)

    for m in start_matches:
        log.info(
            "  START candidate: '%.50s' matched '%s' score=%.0f at %.2f-%.2fs",
            m.text, m.matched_phrase, m.score, m.start, m.end,
        )
    for m in end_matches:
        log.info(
            "  END   candidate: '%.50s' matched '%s' score=%.0f at %.2f-%.2fs",
            m.text, m.matched_phrase, m.score, m.start, m.end,
        )
    if not start_matches:
        log.warning("  No start phrases found in transcript")
    if not end_matches:
        log.warning("  No end phrases found in transcript")

    return start_matches, end_matches


def _find_raw_matches(
    phrase: str,
    words_starts: list[float],
    words_ends: list[float],
    word_strings: list[str],
    threshold: float,
    role: str,
) -> list[_RawMatch]:
    """Find all sliding-window matches for *phrase*, returning word index ranges."""
    results: list[_RawMatch] = []
    phrase_word_count = len(phrase.split())

    for window_size in range(max(1, phrase_word_count - 1), phrase_word_count + 3):
        if window_size > len(word_strings):
            continue
        for i in range(len(word_strings) - window_size + 1):
            # skip windows with large time gaps between consecutive words
            has_gap = False
            for k in range(i, i + window_size - 1):
                if words_starts[k + 1] - words_ends[k] > MAX_WORD_GAP:
                    has_gap = True
                    break
            if has_gap:
                continue

            window_text = " ".join(word_strings[i : i + window_size])
            score = fuzz.ratio(window_text.lower(), phrase.lower())
            if score >= threshold:
                results.append(_RawMatch(
                    word_start=i,
                    word_end=i + window_size,
                    phrase_len=phrase_word_count,
                    match=PhraseMatch(
                        start=words_starts[i],
                        end=words_ends[i + window_size - 1],
                        text=window_text,
                        score=score,
                        matched_phrase=phrase,
                        role=role,
                    ),
                ))
    return results


def _fallback_segment_match(
    segments: list[TranscriptSegment],
    _full_text: str,
    threshold: float,
) -> tuple[list[PhraseMatch], list[PhraseMatch]]:
    """Match against whole segment text when word-level timestamps are unavailable."""
    start_matches: list[PhraseMatch] = []
    end_matches: list[PhraseMatch] = []

    for seg in segments:
        seg_lower = seg.text.lower()
        for phrase in START_PHRASES:
            score = fuzz.partial_ratio(seg_lower, phrase)
            if score >= threshold:
                start_matches.append(PhraseMatch(
                    start=seg.start,
                    end=seg.end,
                    text=seg.text,
                    score=score,
                    matched_phrase=phrase,
                    role="start",
                ))
        for phrase in END_PHRASES:
            score = fuzz.partial_ratio(seg_lower, phrase)
            if score >= threshold:
                end_matches.append(PhraseMatch(
                    start=seg.start,
                    end=seg.end,
                    text=seg.text,
                    score=score,
                    matched_phrase=phrase,
                    role="end",
                ))

    return start_matches, end_matches
